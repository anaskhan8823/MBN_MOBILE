...
class _MapsViewState extends State<MapsView> {
      ...
      appBar: AppBar(
        ...
        bottom: PreferredSize(
          ...
          child: Padding(
            ...
            child: BlocBuilder<MapStoresCubit, MapStoresState>(
              builder: (context, state) {
                // Pass shops/reps dynamically to the search
                return CustomSearchAnchorSearch(
                  representativeList: state.selected == MapFilter.shops,
                );
              },
            ),
          ),
  ...
  Widget _buildCustomCardShop(String markerId) {
    final store = context.read<MapStoresCubit>().state.listOfStores.firstWhere(
        (store) =>
            LatLng(
              double.parse("${store.location?.latitude}"),
              double.parse("${store.location?.longitude}"),
            ).toString() ==
            markerId,
        orElse: () => MapStoresModel());
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.end,
              children: [
                SvgPicture.asset(AppSvg.items, color: Colors.white, height: 10),
              ],
            ),
          ],
        ),
